...
public class BitcoinAddressValidator 
{
    ...
    public static boolean validateBitcoinAddress(String addr) {
        ...
        for(byte bite : decoded)
            decode.append(Integer.toString(bite));
 
        byte[] hash1 = sha256(Arrays.copyOfRange(decoded, 0, 21));
        byte[] hash2 = sha256(hash1);
        
        StringBuilder decodeHash = new StringBuilder();
        for(byte bite : hash2)
            decodeHash.append(Integer.toString(bite));
        
        System.out.println("Address: " + addr);
        System.out.println("Hash1: " + Arrays.toString(Arrays.copyOfRange(hash2, 0, 4)));
        System.out.println("Decoded: " + Arrays.toString(Arrays.copyOfRange(decoded, 21, 25)));
        System.out.println("Decoded string: " + decode);
        System.out.println("Decoded hash string: " + decodeHash);
